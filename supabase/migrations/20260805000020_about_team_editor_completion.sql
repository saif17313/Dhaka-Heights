...
-- Fix 05: complete the dynamic About-page team editor and public profile panel.
-- This migration is safe after Fix 02 and also repairs installations where the
-- earlier team migration file exists locally but was not pushed to Supabase.
...
BEGIN
    ...
    SELECT p_section_id,item->>'itemKey',btrim(item->>'name'),btrim(item->>'role'),btrim(item->>'biography'),NULLIF(item->>'mediaId','')::UUID,btrim(item->>'imageAlt'),ordinal*10,(item->>'isVisible')::BOOLEAN,p_actor
