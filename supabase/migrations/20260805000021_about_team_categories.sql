...
-- Fix 06: add dynamic, ordered Team categories to the About-page CMS.
-- Existing team members are preserved and assigned to the initial
-- "Leadership and Management" category when they do not already have one.
...
SET settings = jsonb_set(
    COALESCE(settings, '{}'::jsonb),
    '{teamSection,modalHeading}',
    to_jsonb(COALESCE(NULLIF(settings#>>'{teamSection,modalHeading}', ''), 'OUR TEAM')),
    true
)
WHERE section_key = 'about-page';
    ...
    LOOP
        SELECT item_key
         ...
         WHERE section_id = v_section.id
           AND item_key LIKE 'team-category-%'
         ORDER BY sort_order, id
         LIMIT 1;

        IF v_category_key IS NULL THEN
            v_category_key := 'team-category-leadership-management';
            INSERT INTO section_items(
                section_id, item_key, title, body_text, sort_order, is_visible
            ) VALUES (
                v_section.id,
                v_category_key,
        ...
        UPDATE section_items member
           SET tag_text = v_category_key
         WHERE member.section_id = v_section.id
           AND member.item_key LIKE 'team-%'
           AND member.item_key NOT LIKE 'team-category-%'
           AND (
               NULLIF(btrim(member.tag_text), '') IS NULL
               OR NOT EXISTS (
                   SELECT 1
                     FROM section_items category
                    WHERE category.section_id = v_section.id
                      AND category.item_key = member.tag_text
                      AND category.item_key LIKE 'team-category-%'
               )
           );
    END LOOP;
END;
...
SELECT jsonb_build_object(
    ...
    'teamCategories', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
            'itemId',si.id,'itemKey',si.item_key,'title',si.title,
            'description',COALESCE(si.body_text,''),
            'sortOrder',si.sort_order,'isVisible',si.is_visible
        ) ORDER BY si.sort_order,si.id)
        FROM section_items si
        WHERE si.section_id=ps.id AND si.item_key LIKE 'team-category-%'
    ),'[]'::jsonb),
    'teamMembers', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
            'itemId',si.id,'itemKey',si.item_key,'categoryKey',si.tag_text,
            'name',si.title,'role',si.subtitle,
        ...
        FROM section_items si LEFT JOIN media_assets ma ON ma.id=si.image_asset_id
        WHERE si.section_id=ps.id
          AND si.item_key LIKE 'team-%'
          AND si.item_key NOT LIKE 'team-category-%'
    ),'[]'::jsonb),
    'concerns', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
...
BEGIN
    IF p_payload IS NULL OR jsonb_typeof(p_payload)<>'object'
       ...
       OR jsonb_typeof(p_payload->'teamCategories')<>'array'
    ...
    v_count:=jsonb_array_length(p_payload->'teamCategories');
    IF v_count NOT BETWEEN 1 AND 12
       OR (SELECT count(DISTINCT item->>'itemKey') FROM jsonb_array_elements(p_payload->'teamCategories') item)<>v_count THEN
        RAISE EXCEPTION 'About team categories are invalid' USING ERRCODE='22023';
    END IF;
    FOR v_item IN SELECT value FROM jsonb_array_elements(p_payload->'teamCategories') LOOP
        IF COALESCE(v_item->>'itemKey','') !~ '^team-category-[a-z0-9-]{1,55}$'
           OR length(btrim(COALESCE(v_item->>'title',''))) NOT BETWEEN 1 AND 120
           OR length(btrim(COALESCE(v_item->>'description',''))) NOT BETWEEN 0 AND 600
           OR jsonb_typeof(v_item->'isVisible')<>'boolean' THEN
            RAISE EXCEPTION 'One or more About team categories are invalid' USING ERRCODE='22023';
        END IF;
    END LOOP;

    v_count:=jsonb_array_length(p_payload->'teamMembers');
    IF v_count NOT BETWEEN 0 AND 48
    ...
    FOR v_item IN SELECT value FROM jsonb_array_elements(p_payload->'teamMembers') LOOP
        v_id := NULL;
        IF NULLIF(v_item->>'mediaId', '') IS NOT NULL THEN
        ...
        IF COALESCE(v_item->>'itemKey','') !~ '^team-[a-z0-9-]{1,64}$'
           OR COALESCE(v_item->>'itemKey','') LIKE 'team-category-%'
           OR COALESCE(v_item->>'categoryKey','') !~ '^team-category-[a-z0-9-]{1,55}$'
           OR NOT EXISTS (
               SELECT 1
               FROM jsonb_array_elements(p_payload->'teamCategories') category
               WHERE category->>'itemKey' = v_item->>'categoryKey'
           )
           OR length(btrim(COALESCE(v_item->>'name',''))) NOT BETWEEN 1 AND 120
           OR length(btrim(COALESCE(v_item->>'role',''))) NOT BETWEEN 1 AND 120
           OR length(btrim(COALESCE(v_item->>'biography',''))) NOT BETWEEN 0 AND 6000
           OR length(btrim(COALESCE(v_item->>'imageAlt',''))) NOT BETWEEN 0 AND 180
           OR jsonb_typeof(v_item->'isVisible')<>'boolean' THEN
            RAISE EXCEPTION 'One or more About team members are invalid' USING ERRCODE='22023';
        END IF;
        IF v_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM media_assets WHERE id=v_id AND resource_type='image' AND is_archived=false) THEN
            RAISE EXCEPTION 'One or more About team member images are invalid' USING ERRCODE='22023';
        END IF;
        IF (v_item->>'isVisible')::BOOLEAN AND (
            v_id IS NULL
            OR length(btrim(COALESCE(v_item->>'biography',''))) < 1
            OR length(btrim(COALESCE(v_item->>'imageAlt',''))) < 1
        ) THEN
            RAISE EXCEPTION 'Visible About team members require a portrait, biography, and image description' USING ERRCODE='22023';
        END IF;
    END LOOP;
...
BEGIN
    ...
    INSERT INTO section_items(section_id,item_key,title,body_text,sort_order,is_visible,updated_by)
    SELECT p_section_id,item->>'itemKey',btrim(item->>'title'),btrim(item->>'description'),ordinal*10,(item->>'isVisible')::BOOLEAN,p_actor
    FROM jsonb_array_elements(p_payload->'teamCategories') WITH ORDINALITY entries(item,ordinal);

    INSERT INTO section_items(section_id,item_key,title,subtitle,body_text,tag_text,image_asset_id,image_alt,sort_order,is_visible,updated_by)
    SELECT p_section_id,item->>'itemKey',btrim(item->>'name'),btrim(item->>'role'),btrim(item->>'biography'),item->>'categoryKey',NULLIF(item->>'mediaId','')::UUID,btrim(item->>'imageAlt'),ordinal*10,(item->>'isVisible')::BOOLEAN,p_actor
