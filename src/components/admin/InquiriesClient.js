...
import { STATUSES, STATUS_LABELS, TYPE_META, typeMeta } from '@/lib/inquiryTypes';
...
export default function InquiriesClient({ initialInquiries, activeStatus = 'all' }) {
  ...
  const typeLabel = typeFilter === 'all' ? 'All Types' : typeMeta(typeFilter).label;

  const runExport = async (format) => {
    if (exporting || visibleInquiries.length === 0) return;
    setExporting(format); setError('');
    try {
      if (format === 'excel') await downloadInquiriesExcel(visibleInquiries, { statusLabel, typeLabel });
      else await downloadInquiriesPdf(visibleInquiries, { statusLabel, typeLabel });
    } catch {
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-white p-6"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#B59410]">Sales workflow · private records</p><h1 className="font-serif text-2xl font-bold text-[#0B1B3D]">Customer Inquiries Inbox</h1><p className="text-xs text-slate-500">Contact messages, project inquiries, callbacks, layout requests, buyer enquiries, and landowner submissions.</p></div><div className="flex flex-wrap items-center gap-2"><div className="rounded-xl border bg-slate-50 px-4 py-2 text-xs font-bold">{visibleInquiries.length} of {inquiries.length} records</div><button type="button" disabled={!!exporting || visibleInquiries.length === 0} onClick={() => runExport('excel')} className="inline-flex items-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-xs font-bold text-emerald-700 disabled:opacity-40"><i className="fa-solid fa-file-excel"></i>{exporting === 'excel' ? 'Preparing…' : 'Download Excel'}</button><button type="button" disabled={!!exporting || visibleInquiries.length === 0} onClick={() => runExport('pdf')} className="inline-flex items-center gap-2 rounded-xl border border-rose-300 bg-rose-50 px-4 py-2 text-xs font-bold text-rose-700 disabled:opacity-40"><i className="fa-solid fa-file-pdf"></i>{exporting === 'pdf' ? 'Preparing…' : 'Download PDF'}</button></div></header>
