  ...
  const body = <main style={{ marginTop: previewMode ? 0 : '80px' }}>
    ...
    <section className="py-20 bg-gray-50 career-listings-section" style={{ backgroundColor: '#fcfcfd', borderTop: '1px solid var(--border-light)' }}>
      <div className="container grid-2 gap-10">
        <div><span className="section-tag">{content.jobsSection.tag}</span><h2 className="section-title text-navy text-2xl font-serif mb-8">{content.jobsSection.heading}</h2><div className="flex flex-col gap-6">{jobs.map((job) => <div key={job.jobId} className="career-job-card"><span className={`career-job-tag ${job.tagClass}`}>{job.department}</span><h4 className="text-navy font-bold text-lg mt-3 mb-1">{job.title}</h4><div className="flex gap-4 text-gray-400 text-xs mb-3 mt-1"><span><PublicIcon iconClass="fa-solid fa-location-dot mr-1" /> {job.location}</span><span>•</span><span><PublicIcon iconClass="fa-solid fa-briefcase mr-1" /> {job.experience}</span></div><p className="text-gray-500 text-sm leading-relaxed">{job.description}</p></div>)}</div></div>
