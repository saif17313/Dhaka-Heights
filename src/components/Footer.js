    ...
    <footer className="main-footer">
      <div className="footer-top-container"><div className="container grid-4">
        <div className="footer-col brand-col">
          ...
          <div className="footer-social-links">{socials.map((item) => <a key={item.itemKey} href={item.url} {...linkProps(item.target)} aria-label={item.platformName}><PublicIcon iconClass={`fa-brands ${item.iconKey}`} /></a>)}</div>
        </div>
        ...
        <div className="footer-col address-col">
          ...
          {footer.address ? <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(footer.address)}`} target="_blank" rel="noopener noreferrer" className="footer-address"><PublicIcon iconClass="fa-solid fa-location-dot" /> {footer.address}</a> : <p className="footer-address"><PublicIcon iconClass="fa-solid fa-location-dot" /></p>}
          {footer.phone ? <a href={`tel:${footer.phone.replace(/[^\d+]/g, '')}`} className="footer-contact-item"><PublicIcon iconClass="fa-solid fa-phone" /> Hotline: {footer.phone}</a> : <p className="footer-contact-item"><PublicIcon iconClass="fa-solid fa-phone" /> Hotline:</p>}
          {footer.email ? <a href={`mailto:${footer.email.trim()}`} className="footer-contact-item"><PublicIcon iconClass="fa-solid fa-envelope" /> Email: {footer.email}</a> : <p className="footer-contact-item"><PublicIcon iconClass="fa-solid fa-envelope" /> Email:</p>}
          {footer.website ? <a href={/^https?:\/\//i.test(footer.website.trim()) ? footer.website.trim() : `https://${footer.website.trim()}`} target="_blank" rel="noopener noreferrer" className="footer-contact-item"><PublicIcon iconClass="fa-solid fa-globe" /> Web: {footer.website}</a> : <p className="footer-contact-item"><PublicIcon iconClass="fa-solid fa-globe" /> Web:</p>}
        </div>
      </div></div>
