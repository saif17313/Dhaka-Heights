    ...
    <footer className="main-footer">
      <div className="footer-top-container"><div className="container grid-4">
        ...
        <div className="footer-col address-col">
          ...
          {footer.address ? <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(footer.address)}`} target="_blank" rel="noopener noreferrer" className="footer-address"><i className="fa-solid fa-location-dot"></i> {footer.address}</a> : <p className="footer-address"><i className="fa-solid fa-location-dot"></i></p>}
          {footer.phone ? <a href={`tel:${footer.phone.replace(/[^\d+]/g, '')}`} className="footer-contact-item"><i className="fa-solid fa-phone"></i> Hotline: {footer.phone}</a> : <p className="footer-contact-item"><i className="fa-solid fa-phone"></i> Hotline:</p>}
          {footer.email ? <a href={`mailto:${footer.email.trim()}`} className="footer-contact-item"><i className="fa-solid fa-envelope"></i> Email: {footer.email}</a> : <p className="footer-contact-item"><i className="fa-solid fa-envelope"></i> Email:</p>}
          {footer.website ? <a href={/^https?:\/\//i.test(footer.website.trim()) ? footer.website.trim() : `https://${footer.website.trim()}`} target="_blank" rel="noopener noreferrer" className="footer-contact-item"><i className="fa-solid fa-globe"></i> Web: {footer.website}</a> : <p className="footer-contact-item"><i className="fa-solid fa-globe"></i> Web:</p>}
        </div>
      </div></div>
